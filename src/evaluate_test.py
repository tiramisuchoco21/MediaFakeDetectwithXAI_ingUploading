...
def infer(
    ...
    save_crops: bool   = False, # crop 저장 여부 (이미지/영상 모두 지원)
    ...
    crop 저장 예시:
          infer(url, ..., save_crops=True)
          → output_crops/ 폴더에 프레임별 crop 저장
          → 파일명: frame0000_crop0_x1_y1_x2_y2.jpg
    ...
    for frame_idx, frame_bgr in enumerate(frames):
        ...
        if save_crops:
            ...
            print(f"frame {frame_idx}: {len(crops)}개 crop → {crop_dir}/")
...
# 4. 히트맵 영상 저장
def save_heatmap_video(overlays: list, output_path: str, fps: float = 1.0):
    if not overlays:
        print("저장할 프레임 없음")
        return
...
if __name__ == "__main__":
    ...
    # 전체 영상 분석(링크 포함)
    # result = infer("video.mp4", model, vae_extractor, region_cropper, DEVICE)

    # 구간 지정 분석 (10초~20초)
    # result = infer("video.mp4", model, vae_extractor, region_cropper, DEVICE, start_time=10.0, end_time=20.0)

    # 이미지 분석 + crop 저장
    #result = infer("SJW.jpg", model, vae_extractor, region_cropper, DEVICE, save_crops=True)

    # 영상 분석 + crop 저장
    #result = infer("https://www.youtube.com/shorts/nTsOh2w_JHE", model, vae_extractor, region_cropper, DEVICE, save_crops=True, crop_dir="output_crops")

    # 구간 지정 + crop 저장
    result = infer("https://www.youtube.com/shorts/ZiPUKD6G4V0?feature=share",
                    model,
                    vae_extractor,
                    region_cropper,
                    DEVICE,
